...
# 1. Pobieramy WIĘCEJ danych (od 2000) + VIX (Indeks Strachu)
...
VIX_CACHE_FILE = os.path.join(DATA_DIR, 'VIX_cache.csv')
if os.path.exists(VIX_CACHE_FILE):
    vix = pd.read_csv(VIX_CACHE_FILE, index_col=0, parse_dates=True)
else:
...
# 2. Feature Engineering
# Używamy już wczytanych zmiennych df i vix
...
if isinstance(vix, pd.DataFrame):
    if 'Close' in vix.columns:
        ...
        # Fallback if no Close column or just one column
        vix = vix.iloc[:, 0] if not vix.empty else pd.Series(dtype=float)

# Handle empty VIX (if download failed for good)
if vix.empty:
    print("WARNING: VIX data is empty. Using fill value 20.0")
    df['VIX'] = 20.0
else:
    df['VIX'] = vix

df['VIX'] = df['VIX'].ffill().bfill() # Uzupełniamy braki
...
results.to_csv(os.path.join(STAGES_DIR, 'step4_predictions.csv'))
...
joblib.dump(model, os.path.join(STAGES_DIR, 'step4_model_xgboost.joblib'))
data_step4 = {
    'X_train': X_train,
    'X_test': X_test,
    'y_train': y_train,
...
joblib.dump(data_step4, os.path.join(STAGES_DIR, 'step4_data.pkl'))
