...
df = pd.read_csv(os.path.join(STAGES_DIR, 'step4_predictions.csv'), index_col=0, parse_dates=True)
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(PLOTS_DIR, 'step5_equity_curve.png'), dpi=300, bbox_inches='tight')
...
joblib.dump(meta_model, os.path.join(STAGES_DIR, 'step5_model_meta_rf.joblib'))
data_step5 = {
    'X_m_train': X_m_train,
    'X_m_test': X_m_test,
    'y_m_train': y_m_train,
...
joblib.dump(data_step5, os.path.join(STAGES_DIR, 'step5_data.pkl'))
